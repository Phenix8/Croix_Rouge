...
namespace PixelCrushers.DialogueSystem.SequencerCommands
{
    ...
    public class SequencerCommandPlayNormalSound : SequencerCommand

    {
        ...
        void InstanciateSound(string eventName, float delay, float volume, int intBoolLoop, Vector3 pos, Vector3 endPos, float tweenDuration)
        {
            ...
            AudioSource newEmitter = newGO.AddComponent<AudioSource>() as AudioSource;
            newEmitter.clip = Resources.Load(eventName) as AudioClip;
            newEmitter.volume = volume;
            newEmitter.spatialBlend = 1;
            newEmitter.rolloffMode = AudioRolloffMode.Linear;

            if(intBoolLoop == 0)
            {
                newEmitter.loop = true;
            }
            //prefab = Resources.LoadAssetAtPath<GameObject>("Assets/Artwork/mymodel.fbx");
            if (endPos != Vector3.zero)
            {
                
                StartCoroutine(SoundCouroutine(newEmitter, delay, endPos, tweenDuration));
            }
            else
            {
                StartCoroutine(SoundCouroutine(newEmitter, delay, endPos, tweenDuration));
        ...
        IEnumerator SoundCouroutine(AudioSource emitter, float delay, Vector3 endPos, float tweenDuration)
        {
